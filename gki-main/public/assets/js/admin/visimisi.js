...
$(document).ready(function() {
		tinymce.init({
        selector: 'textarea#title',  // Targetkan textarea yang akan digunakan oleh TinyMCE
        plugins: 'image code link',
        toolbar: 'undo redo | image | link | code',
        images_upload_url: base_url + 'visimisi/upload_image',  // URL untuk mengunggah gambar
        ...
        images_upload_handler: function (blobInfo, success, failure) {
            ...
            $.ajax({
                url: base_url + 'visimisi/upload_image',
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
  ...
  var dataTable = $('#tabel_serverside').DataTable({
    ...
    "scrollX": true,
    "ajax": {
      "url": base_url + "visimisi/get_list",
      "type": "post",
      "dataType": 'json',
      "data": {},
    },
    ...
    Swal.fire({
        ...
        didOpen: () => {
            ...
            tinymce.init({
                ...
                plugins: 'image code link',
                toolbar: 'undo redo | image | link | code',
                images_upload_url: base_url + 'visimisi/upload_image',
                automatic_uploads: true,
            });
            tinymce.init({
                selector: 'textarea#title',  // Targetkan textarea body
                plugins: 'image code link',
                toolbar: 'undo redo | image | link | code',
                images_upload_url: base_url + 'visimisi/upload_image',
                automatic_uploads: true,
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                type: "POST",
                url: base_url + 'visimisi/add',  // URL endpoint untuk menambah konten
                data: {
                    visi: result.value.title,
                    misi: result.value.body
                },
                success: function(response) {
...
$(document).on('click', '.edit', function() {
    ...
    $.ajax({
        url: base_url + 'visimisi/get/' + kontenId,
        method: 'GET',
        dataType: 'json',
            ...
            Swal.fire({
                ...
                html: `
                    <form id="form_edit_data">
                        <div class="form-group">
                            <label for="title">Visi</label>
                            <textarea class="form-control" id="title">${konten.visi}</textarea>
                        </div>
                        <div class="form-group">
                            <label for="body">Misi</label>
                            <textarea class="form-control" id="body">${konten.misi}</textarea>
                        </div>
                    </form>
                `,
                didOpen: () => {
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        ...
                        success: function() {
                            Swal.fire('Sukses', 'Visi Misi berhasil diperbarui.', 'success');
                            $('#tabel_serverside').DataTable().ajax.reload();  // Reload DataTables
                        },
                        error: function() {
                            Swal.fire('Error', 'Gagal memperbarui Visi Misi.', 'error');
                        }
                    });
                }
            });
        ...
        error: function() {
            Swal.fire('Error', 'Gagal memuat data Visi Misi.', 'error');
        }
    });
});
    ...
    $(document).on('click', '.delete', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "POST",
                    url: base_url + '/visimisi/delete/' + kontenId,
                    success: function() {
                        Swal.fire('Sukses', 'Visi Misi berhasil dihapus.', 'success');
                       $('#tabel_serverside').DataTable().ajax.reload();  // Reload DataTables
                    },
                    error: function() {
                        Swal.fire('Error', 'Gagal menghapus Visi Misi.', 'error');
                    }
                });
            }
        });

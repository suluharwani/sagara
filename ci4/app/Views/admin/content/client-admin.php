<div class="content-header">
    <div class="d-flex justify-content-between align-items-center">
        ...
        <button class="btn btn-admin btn-admin-primary" data-bs-toggle="modal" data-bs-target="#addClientModal">
                            ...
                            <?php if (!empty($clients)): ?>
                                <?php $no = 1; foreach ($clients as $client): ?>
                                    <tr>
                                        <td><?= $no++ ?></td>
                                        <td>
                                            <strong><?= esc(($client['nama_depan'] ?? '') . ' ' . ($client['nama_belakang'] ?? '')) ?></strong>
                                        </td>
                                        <td><?= esc($client['email'] ?? '-') ?></td>
                                        <td>
                                            <?php if (($client['status'] ?? 0) == 1): ?>
                                                <span class="badge bg-success">Active</span>
                                            <?php else: ?>
                                                <span class="badge bg-secondary">Inactive</span>
                                            <?php endif; ?>
                                        </td>
                                        <td><?= date('d M Y', strtotime($client['created_at'] ?? 'now')) ?></td>
